...
interface AvulsoSkuManagerProps {
  onClose: () => void;
  avulsos: any[];
  onAvulsosUpdated: (avulsos: any[]) => void;
}

export function AvulsoSkuManager({ onClose, avulsos, onAvulsosUpdated }: AvulsoSkuManagerProps) {
  ...
  const patterns = settings.avulsoSkuPatterns || [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedAvulsos = [...avulsos];

      if (autoApply) {
        const batch = writeBatch(db);
        for (let i = 0; i < updatedAvulsos.length; i++) {
          const p = updatedAvulsos[i];
          if (p.category === category) {
              const newSku = `${prefix}${String(currentNum).padStart(3, '0')}`;
              currentNum++;
              hasChanges = true;
              batch.set(doc(db, 'avulso_products', String(p.id)), { sku: newSku }, { merge: true });
              updatedAvulsos[i] = { ...p, sku: newSku };
          }
        }
        if (hasChanges) {
          await batch.commit();
          onAvulsosUpdated(updatedAvulsos);
        }
      }
      ...
      await updateSettings({ avulsoSkuPatterns: newPatterns });
